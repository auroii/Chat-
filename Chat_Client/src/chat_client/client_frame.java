...
//declaracao das biblitecas utilizadas
import java.net.*;
import java.io.*;
import java.util.*;
...
{
    //nome do usuario e seu respectivo endereco
    String nomeUsuario, endereco = "localhost";
    ArrayList<String> usuarios = new ArrayList(); //lista de usuarios no cliente
    int porta = 2222;//porta para o socket
    Boolean conectado = false; //variavel flag
    
    Socket sock; //sock, leitor e escritor de dados para o server
    BufferedReader leitor;
    PrintWriter escritor;
    
    //--------------------------//
    
    //thread para recever dados
    public void ListenThread() 
    {
         Thread IncomingReader = new Thread(new leitorEntradaTexto());
    ...
    /**
     * 
     * @param data nome do usuario a ser adicionado ao cliente
     * 
     */
    ...
    /**
     * 
     * @param data nome do usuario a ser removido do cliente 
     */
    public void removerUsuario(String data) 
    {
         ta_chat.append(data + " foi desligado.\n");
    }
    ...
    /**
     * Quando o usuario sai do chat e feito um tratamento amigavel 
     * escrendo na tela e depois limpando o buffer de escrita
     */
    public void desconectarMsg() 
    {
        String bye = (nomeUsuario + ": :Desconectado");
        try
        {
            escritor.println(bye); 
            escritor.flush(); 
        } catch (Exception e) 
        {
            ta_chat.append("Nao foi possivel enviar a mensagem.\n");
        }
    }
    ...
    public void desconectar() 
    {
        try 
        {
            ...
            ta_chat.append("Falha em desconectar \n");
        }
        conectado = false;
    ...
    /**
     * classe leitorEntradaTexto, que le o que o cliente recebe em tempo real, implementa
     * a interface Runnable
     */
    public class leitorEntradaTexto implements Runnable
    {
        @Override
        public void run() 
        {
            ...
            try 
            {
                ...
                while ((stream = leitor.readLine()) != null) 
                {
                     data = stream.split(":");
    ...
    //--------------------------//
    //Acoes padrao para implementat as funcionalidades dos botoes na GUI
    ...
    private void b_connectActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_b_connectActionPerformed
        if (conectado == false) 
        {
            nomeUsuario = tf_username.getText();
            tf_username.setEditable(false);

            try 
            {
                sock = new Socket(endereco, porta);
                InputStreamReader entradaDados = new InputStreamReader(sock.getInputStream());
                leitor = new BufferedReader(entradaDados);
                escritor = new PrintWriter(sock.getOutputStream());
                escritor.println(nomeUsuario + ":has connected.:Connect");
                escritor.flush(); 
                conectado = true; 
            } 
            catch (Exception ex) 
    ...
    /**
     * implementacao do botao anonimo
     * @param evt evento para caso haja cadastro de um usuario anonimo
     */
    private void b_anonymousActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_b_anonymousActionPerformed
        ...
        if (conectado == false) 
        {
            ...
            try 
            {
                ...
                escritor = new PrintWriter(sock.getOutputStream());
                escritor.println(anon + ":has connected.:Connect");
                escritor.flush(); 
                conectado = true; 
            } 
            catch (Exception ex) 
    ...
    private void b_sendActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_b_sendActionPerformed
        String vazia = "";
        if ((tf_chat.getText()).equals(vazia)) {
            tf_chat.setText("");
            tf_chat.requestFocus();
        } else {
            try {
               escritor.println(nomeUsuario + ":" + tf_chat.getText() + ":" + "Chat");
               escritor.flush(); // flushes the buffer
            } catch (Exception ex) {
    ...
    {
        //invoker de evento executando uma classe anonima para rodar o frame
        //do cliente
